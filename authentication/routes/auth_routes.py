...
from schemas.user_schema import LoginRequest, RegisterRequest, BaseResponse, LoginWithGoogleRequest, TokenDecodeRequest, TokenRefreshRequest
...
@router.post("/login")
def login(user: LoginRequest, app_id: str = Header(None, convert_underscores=False)):
    try:
...
def decode_token(token_request: TokenDecodeRequest, app_id: str = Header(None, convert_underscores=False)):
    try:
...
@router.post("/refresh")
def refresh_token(request: TokenRefreshRequest, app_id: str = Header(None, convert_underscores=False)):
    try:
...
def register(user_data: RegisterRequest, app_id: str = Header(None, convert_underscores=False)):
    try:
        new_user = service.register(user_data.dict(), app_id)
        return BaseResponse(status_code=201, data=new_user, message="User registered successfully")
    except Exception as e:
        return BaseResponse(status_code=400, message=str(e))   
    
@router.post("/login/google")
async def login_with_google(login: LoginWithGoogleRequest, app_id: str = Header(None, convert_underscores=False)):
    try:
