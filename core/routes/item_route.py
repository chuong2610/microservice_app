...
def get_item_by_id(item_id: str):
    try:
        ...
        if item:
            return BaseResponse(status_code=200, message="Item retrieved successfully", data=item.dict())
        else:
...
def create_item(item_data: dict):
    try:
        ...
        return BaseResponse(status_code=201, message="Item created successfully", data=new_item.dict())
    except Exception as e:
        return BaseResponse(status_code=500, message=str(e), data=None)    
...
def update_item(item_id: str, update_data: dict):
    try:
        ...
        if updated_item:
            return BaseResponse(status_code=200, message="Item updated successfully", data=updated_item.dict())
        else:
            return BaseResponse(status_code=404, message="Item not found", data=None)
    except Exception as e:
        return BaseResponse(status_code=500, message=str(e), data=None)
